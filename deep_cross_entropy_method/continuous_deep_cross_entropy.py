...
class DeepCrossEntropyAgent:
    ...
    def choose_action(self, state):
        ...
        action_logits = torch.tanh(logits)
        action = action_logits + self.epsilon * torch.randn(self.action_dim)
        return action
    ...
    def compute_loss(self):
        ...
        for states, actions in zip(self.elite_states, self.elite_actions):
            ...
            loss = torch.mean((action_logits - actions) ** 2)
            losses.append(loss)
...
def render_episode():
    ...
    agent = DeepCrossEntropyAgent(
        ...
        num_trajectories=50,
        elite_quantile=0.8,
        policy_hidden_units=[150, 100],
    )

    agent.policy_net.load_state_dict(
        torch.load("deep_cross_entropy_method/continuous_policy.pth")
    )
...
def main():
    ...
    torch.save(
        agent.policy_net.state_dict(), "deep_cross_entropy_method/continuous_policy.pth"
    )
